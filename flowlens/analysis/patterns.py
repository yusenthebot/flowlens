...
def _detect_retry_storm(trace: Trace, threshold: int | None = None) -> list[DetectedPattern]:
    ...
    for name, count in name_counts.items():
        if count >= threshold:
            ...
            error_rate = (
                sum(1 for s in tool_spans if s.name == name and s.status == SpanStatus.ERROR)
                / count
            )
...
def _detect_infinite_loop(trace: Trace, max_repeat: int | None = None) -> list[DetectedPattern]:
    """
    ...
    tool_sequence = [
        s.name for s in sorted(trace.spans, key=lambda s: s.start_time) if s.kind == SpanKind.TOOL
    ]
            ...
            if repeat_count >= max_repeat:
                ...
                patterns.append(
                    DetectedPattern(
                        ...
                        severity="critical",
                        description=(
                            f"检测到循环模式 {' → '.join(cycle)} " f"重复 {repeat_count} 次"
                        ),
...
def _detect_context_overflow(
    ...
    threshold_ratio = (
        threshold_ratio if threshold_ratio is not None else settings.pattern_context_ratio
    )
...
def _detect_timeout_cascade(trace: Trace, dag: CausalDAG) -> list[DetectedPattern]:
    """检测超时导致的级联失败"""
    timeout_spans = [
        s
        for s in trace.spans
        if s.status == SpanStatus.ERROR and s.error_message and "timeout" in s.error_message.lower()
    ]
        ...
        if cascaded:
            patterns.append(
                DetectedPattern(
                    ...
                    severity="critical",
                    description=(f"'{ts.name}' 超时后导致 {len(cascaded)} 个下游操作失败"),
...
def _detect_hallucination_cascade(trace: Trace, dag: CausalDAG) -> list[DetectedPattern]:
    """
...
def _detect_cost_spike(
    ...
    """
    cost_spike_ratio = (
        cost_spike_ratio if cost_spike_ratio is not None else settings.pattern_cost_spike_ratio
    )
...
def _detect_slow_tool(
    ...
    """
    slow_tool_multiplier = (
        slow_tool_multiplier
        if slow_tool_multiplier is not None
        else settings.pattern_slow_tool_multiplier
    )
    tool_spans = [s for s in trace.spans if s.kind == SpanKind.TOOL]
    if len(tool_spans) < 2:
        return []
...
def _detect_token_waste(
    ...
    """
    ratio_threshold = (
        ratio_threshold if ratio_threshold is not None else settings.pattern_token_waste_ratio
    )
    patterns: list[DetectedPattern] = []
...
def _detect_sequential_bottleneck(trace: Trace) -> list[DetectedPattern]:
    ...
    for parent_id, tools in parent_to_tools.items():
        ...
        total_sequential_ms = sum(s.duration_ms for s in independent)
...
def _filter_independent_spans(spans: list[Span]) -> list[Span]:
    ...
    for span in spans:
        ...
        references_sibling = any(sid in attrs_str for sid in span_ids if sid != span.span_id)
    ...
    for parent_id, siblings in parent_to_children.items():
        ...
        for span in sorted_siblings:
            if span.status == SpanStatus.ERROR:
                ...
                recovery_time_ms = (
                    (span.end_time - failed_span.start_time) * 1000 if span.end_time > 0 else 0.0
                )
                patterns.append(
                    DetectedPattern(
        ...
        if len(span_ids) > 1:
            patterns.append(
                DetectedPattern(
                    ...
                    severity="info",
                    description=(
                        f"TOOL '{tool_name}' 被相同参数调用了 {len(span_ids)} 次，" f"可能造成浪费"
                    ),
